            ...
            for (int y = minY; y <= maxY; y++)
            {
                ...
                for (int x = minX; x <= maxX; x++)
                {
                    ...
                    if ((index >= 0) && (index < _TileInfoCache.Length))
                    {
                        ...
                        Texture2D test = new Texture2D(d.GraphicsDevice, tileWidth,tileHeight);
                        test = GetTileTexture(index+1, tilesets[0], test);
                        d.Draw(new UITexture(test), destPos - viewPos, info.Rectangle,
        ...
        public void Draw(IBatchedDrawingService d, Rectangle rectangle, Vector2 offset, Vector2 viewportPosition, float opacity)
        {
            ...
            if (this.X + offset.X + this.Width > minX && this.X + offset.X < maxX)
                if (this.Y + offset.Y + this.Height > minY && this.Y + offset.Y < maxY)
                {
                    ...
                    d.Draw(new UITexture(_Texture), new Rectangle(x, y, this.Width, this.Height), new Rectangle(0, 0, _Texture.Width, _Texture.Height), Color.White * opacity,0f,Vector2.Zero,SpriteEffects.None,0f);
                }
